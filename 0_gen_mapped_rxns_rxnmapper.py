#!/usr/bin/env python
"""
Step 0b — Generate atom-mapped reaction SMILES using RXNMapper.

Reads rxn_data.csv (produced by 0_generate_reaction_fp.py) and produces
mapped_rxns.csv with columns: id, rxn_smiles, mapped_rxn, confidence.

Spawns one worker process per available GPU for parallel mapping.

Usage (from the rxnfp conda environment):
    conda activate rxnfp
    python 0_generate_mapped_rxns.py
"""
...
import torch
from config import BASE_DIR, RXN_DATA_CSV
...
def worker(gpu_id, reactions, result_queue):
    ...
    for i in range(0, total, BATCH_SIZE):
        ...
        try:
            ...
            # Batch failed — retry each reaction individually
...
if __name__ == "__main__":
    ...
    # Split reactions evenly across GPUs
    ...
    # Re-sort by original order (by reaction id as they appear in rxn_data.csv)
