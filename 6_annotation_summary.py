...
# ─────────────────────────────────────────────────────────────────────────────
# Figure 1: EC Annotation of Unannotated Reactions
# ─────────────────────────────────────────────────────────────────────────────
...
def make_annotation_figure(df_rxn, df_u, output_path):
    ...
    # Filter to valid prefilter predictions with n>=3
    has_pf = df_u[
    ...
    fig.suptitle(
        "EC Number Annotation of Unannotated Reactions",
        fontsize=15,
        fontweight="bold",
        y=0.98,
    )
    ...
    # New predictions (prefilter, n>=3, conf>=0.6)
    ...
    ax.set_title(
        "Existing Annotations vs New Predictions\nby EC Class",
        fontsize=12,
        fontweight="bold",
        pad=10,
    )
    ...
    for label, (lo, hi) in tiers:
        cnt = len(
            has_pf[
                (has_pf["prefilter_confidence"] >= lo)
                & (has_pf["prefilter_confidence"] < hi)
            ]
        )
    ...
    ax.set_title(
        "Predicted Reactions by Agreement Level\n(prefilter, n>=3, agreement>=60%)",
        fontsize=12,
        fontweight="bold",
        pad=10,
    )
    ...
    # Stats computed on predicted reactions (>=60% agreement)
    predicted_pf = has_pf[has_pf["prefilter_confidence"] >= 0.6]
    mean_conf = (
        predicted_pf["prefilter_confidence"].mean() * 100
        if len(predicted_pf) > 0
        else 0
    )
    median_conf = (
        predicted_pf["prefilter_confidence"].median() * 100
        if len(predicted_pf) > 0
        else 0
    )
    mean_cos = predicted_pf["prefilter_cos_mean"].mean() if len(predicted_pf) > 0 else 0
    mean_nann = (
        predicted_pf["prefilter_n_annotated_neighbours"].mean()
        if len(predicted_pf) > 0
        else 0
    )
    ...
    summary_lines = [
        ...
        (
            ...
            f"{n_annotated:,} ({100 * n_annotated / total_rxns:.1f}%)",
        ),
        (
            "Previously unannotated",
            f"{n_unannotated:,} ({100 * n_unannotated / total_rxns:.1f}%)",
        ),
        ("", ""),
        ("With reaction center extracted", f"{n_with_center:,}"),
        (
            "Predicted (n>=3, agreement>=60%)",
            f"{n_predicted:,} ({100 * n_predicted / n_unannotated:.1f}% of unannotated)",
        ),
        (
            "Very high agreement (>=80%)",
            f"{n_high_80:,} ({100 * n_high_80 / n_unannotated:.1f}% of unannotated)",
        ),
        (
            "Highest agreement (>=90%)",
            f"{n_high_90:,} ({100 * n_high_90 / n_unannotated:.1f}% of unannotated)",
        ),
        ("", ""),
        ...
        (
            "New annotation rate",
            f"{100 * n_predicted / total_rxns:.1f}% of all reactions gain EC",
        ),
    ]
...
# ─────────────────────────────────────────────────────────────────────────────
# Figure 2: Misannotation Correction
# ─────────────────────────────────────────────────────────────────────────────
...
def make_misannotation_correction_figure(df_mis, df_rxn, output_path):
    ...
    fig, axes = plt.subplots(2, 2, figsize=(14, 10))
    fig.suptitle(
        f"Misannotation Detection and Correction (n={len(df_mis):,} candidates)",
        fontsize=15,
        fontweight="bold",
        y=0.98,
    )
    ...
    # Further break down: same class = subclass error, diff class = class error
    conf_tiers = {
        "50-60%": ((0.5, 0.6), []),
        "60-70%": ((0.6, 0.7), []),
        "70-80%": ((0.7, 0.8), []),
    ...
    tier_same = [
        sum(1 for r in lst for _ in [1] if r["same_class"])
        for _, (_, lst) in conf_tiers.items()
    ]
    tier_diff = [
        sum(1 for r in lst for _ in [1] if not r["same_class"])
        for _, (_, lst) in conf_tiers.items()
    ]
    ...
    ax.bar(
        ...
        tier_same,
        w,
        label=f"Within-class error ({n_same})",
        color=COL_P,
        edgecolor="white",
        linewidth=0.5,
    )
    ax.bar(
        ...
        tier_diff,
        w,
        label=f"Cross-class error ({n_diff})",
        color="#d9534f",
        edgecolor="white",
        linewidth=0.5,
    )
    ...
    ax.set_ylabel("Count", fontsize=10)
    ax.set_title(
        "Misannotation Types by Confidence", fontsize=12, fontweight="bold", pad=10
    )
    ax.legend(fontsize=9, frameon=False)
    ...
    # Legend
    legend_elements = [
        mpatches.Patch(facecolor=COL_G, label="Within-class"),
        mpatches.Patch(facecolor="#d9534f", label="Cross-class"),
    ]
    ...
    x = np.arange(len(all_classes))
    bars = ax.bar(
        x, mis_rates, color=COL_P, edgecolor="white", linewidth=0.5, width=0.6
    )
    ax.set_xticks(x)
    ax.set_xticklabels(
        [f"EC {c}\n{EC1_NAMES.get(c, '')}" for c in all_classes], fontsize=8
    ...
    ax.set_title(
        "Estimated Misannotation Rate by EC Class",
        fontsize=12,
        fontweight="bold",
        pad=10,
    )
    ...
    summary_lines = [
        ...
        (
            ...
            f"{len(df_mis):,} ({overall_mis_rate:.1f}% of annotated)",
        ),
        ("", ""),
        (
            "Within-class errors (same EC class)",
            f"{n_same_total:,} ({100 * n_same_total / len(df_mis):.1f}%)",
        ),
        (
            "Cross-class errors (different EC class)",
            f"{n_diff_total:,} ({100 * n_diff_total / len(df_mis):.1f}%)",
        ),
        ("", ""),
        ("Mean % neighbor agreement", f"{mean_conf:.1f}%"),
        ("Median % neighbor agreement", f"{median_conf:.1f}%"),
...
def main():
    print("Loading data ...")
    df_rxn = pd.read_csv(os.path.join(BASE_DIR, "rxn_data.csv"))
    ...
    out1 = os.path.join(BASE_DIR, "annotation_summary.png")
    out2 = os.path.join(BASE_DIR, "misannotation_correction_summary.png")

    print("\nGenerating annotation summary ...")
    make_annotation_figure(df_rxn, df_u, out1)

    print("Generating misannotation correction summary ...")
    make_misannotation_correction_figure(df_mis, df_rxn, out2)
