#!/usr/bin/env python
"""
Step 4 — Batch evaluation comparing vanilla FAISS search vs.
...
Usage:
    ...
    python 4_batch_evaluation.py              # full evaluation + plot
    python 4_batch_evaluation.py --plot-only  # regenerate plot from saved CSVs
"""
...
TOP_K = 30
MIN_ANNOTATED = 1  # predict whenever at least 1 annotated neighbour exists; filtering done in 6_annotation_summary.py
MIN_CONFIDENCE = (
    0.0  # no confidence gate here; filtering done in 6_annotation_summary.py
)
N_WORKERS = 70
EVAL_RESULTS_CSV = os.path.join(BASE_DIR, "evaluation_results.csv")
EC_PREDICTIONS_CSV = os.path.join(BASE_DIR, "ec_predictions_unannotated.csv")
MISANNOTATION_CSV = os.path.join(BASE_DIR, "misannotation_candidates.csv")
EVAL_PLOT_PNG = os.path.join(BASE_DIR, "evaluation_comparison.png")
...
def _top_k_search(query_idx, candidate_indices=None, exclude_self=True):
    ...
    If candidate_indices is None, search all reactions via FAISS index.
    If provided, search only within that subset (prefilter) via NumPy.
    """
...
def _majority_vote_ec(indices, sims):
    """Return (predicted_ec, confidence, n_annotated) from majority vote of neighbours' ECs.

    Only neighbours with EC annotations are considered.
    confidence = fraction of annotated neighbours that agree on the predicted EC.
    When ECs are tied on frequency, the one whose most-similar neighbour has the
    highest cosine similarity is preferred.
    Returns (None, 0.0, n_annotated) if no annotated neighbours exist.
    """
    ec_counter = Counter()
    ec_best_sim = {}
    n_annotated = 0
...
def _predict_ecs(indices, threshold=0.2):
    ...
    Returns (predicted_ecs, ec_confidences, n_annotated).
    Returns ([], {}, n_annotated) if no annotated neighbours exist.
    """
    ec_counter = Counter()
    n_annotated = 0
    for idx in indices:
...
def make_plots(annotated_results, unannotated_results, misannotations, output_path):
    ...
    v_cov = sum(
        1 for r in unannotated_results if _has_prediction(r["vanilla_predicted_ec"])
    )
    p_cov = sum(
        1 for r in unannotated_results if _has_prediction(r["prefilter_predicted_ec"])
    )
    v_conf_list = [
        r["vanilla_confidence"]
        for r in unannotated_results
        if _has_prediction(r["vanilla_predicted_ec"])
    ]
    p_conf_list = [
        r["prefilter_confidence"]
        for r in unannotated_results
        if _has_prediction(r["prefilter_predicted_ec"])
    ]
    ...
    ax.set_title(
        f"Unannotated Reactions (n={total_u:,})", fontsize=12, fontweight="bold", pad=10
    )
    ...
    ax.hist(
        [c * 100 for c in v_conf_list],
        bins=bins,
        alpha=1.0,
        label=f"Without Prefilter (n={len(v_conf_list):,})",
        color=COL_V,
        edgecolor="white",
        linewidth=0.3,
    )
    ax.hist(
        [c * 100 for c in p_conf_list],
        bins=bins,
        alpha=0.85,
        label=f"With Prefilter (n={len(p_conf_list):,})",
        color=COL_P,
        edgecolor="white",
        linewidth=0.3,
    )
    ...
    if misannotations:
        ...
        ax.barh(
            y_pos,
            counts,
            color=plt.cm.Set2(np.linspace(0, 0.9, len(sorted_classes))),
            edgecolor="white",
            linewidth=0.5,
            height=0.6,
        )
        ax.set_yticks(y_pos)
        ax.set_yticklabels(
            [f"EC {c} {ec1_names.get(c, '')}" for c in sorted_classes], fontsize=9
        )
        ax.invert_yaxis()
        ax.set_xlabel("Count", fontsize=10)
        ax.set_title(
            f"Misannotation Candidates by EC Class\n(n={len(misannotations):,}, zero overlap with predicted ECs)",
            fontsize=12,
            fontweight="bold",
            pad=10,
        )
    ...
    fig.subplots_adjust(
        top=0.93, bottom=0.08, left=0.07, right=0.97, hspace=0.45, wspace=0.35
    )
...
def _run_evaluation():
    ...
    _FPS_NORMED, _CENTER_TO_INDICES, _FAISS_INDEX = (
        fps_normed,
        dict(center_to_indices),
        faiss_index,
    )

    annotated_idx = [i for i in range(len(ids)) if ec_lists[i] and centers[i] != "[]"]
    unannotated_idx = [
        i for i in range(len(ids)) if not ec_lists[i] and centers[i] != "[]"
    ]
    ...
    def _run_pool(worker, indices, label):
        ...
        with Pool(N_WORKERS) as pool:
            for count, r in enumerate(
                pool.imap_unordered(worker, indices, chunksize=8), start=1
            ):
                if r is not None:
                    results.append(r)
                if count % 2000 == 0:
                    ...
                    print(
                        f"  {label}: {count:>6d} / {len(indices)} ({count / len(indices) * 100:.1f}%)  [{rate:.0f} rxn/s, ETA {eta:.0f}s]"
                    )
                    sys.stdout.flush()
        print(
            f"  {label}: {len(indices)} / {len(indices)} done ({time.time() - t:.1f}s)\n"
        )
        return results
    ...
    for metric, label in [
        ...
        delta = p - v
        print(
            f"  {label:<30s}  Vanilla: {v:.4f}  Prefilter: {p:.4f}  ({'+' if delta >= 0 else ''}{delta:.4f})"
        )
    print()
    ...
    print(
        f"  Reactions with prediction:  Vanilla: {v_covered}/{total_u} ({v_covered / total_u * 100:.1f}%)  Prefilter: {p_covered}/{total_u} ({p_covered / total_u * 100:.1f}%)"
    )
    print(
        f"  Mean % neighbor agreement:  Vanilla: {np.mean(v_conf):.4f}  Prefilter: {np.mean(p_conf) if p_conf else 0:.4f}"
    )
    print(
        f"  High-agreement (>=0.5):     Vanilla: {sum(1 for c in v_conf if c >= 0.5)}  Prefilter: {sum(1 for c in p_conf if c >= 0.5)}"
    )
    print()
    ...
    for r in annotated_results:
        ...
        if (
            r["prefilter_f1"] == 0.0
            and r["prefilter_n_predicted_ecs"] > 0
            and conf >= 0.5
        ):
            ...
            # Skip wildcard predictions (e.g. 4.4.1.-) — these only mean the
            # sub-subclass couldn't be resolved, not that the annotation is wrong
            if ".-" in str(top_pred_ec):
                continue

            # Skip if the predicted EC shares the same sub-subclass (EC1.EC2) as
            # any actual EC — this is a specificity granularity difference, not a
            # misannotation (e.g. 1.4.3.3 predicted vs 1.4.3.1 actual)
            ...
            # Skip if the reaction center pool is too large (>200 members) —
            # an overly generic center (e.g. bare phosphate motif or generic
            # amide bond) confounds the prefilter search with many unrelated
            # enzyme families, making the majority-vote unreliable
            if len(cand_indices) > 200:
                continue

            # Skip if there are same-EC-class neighbors in the pool but none
            # of them carry the predicted EC class — this means the predicted
            # EC is driven entirely by a different enzyme family that shares
            # the reaction center motif (e.g. glycosyltransferases co-pooled
            # with transhydrogenases via the nicotinamide-ribose scaffold).
            # When the actual EC class has zero presence in the pool we skip
            # this check (wildcard annotations like 2.3.1.- fall in this case).
            ...
            same_ec1_neighbors = [
                ...
                if idx != query_idx
                and any(ec.split(".")[0] in actual_ec1s for ec in _EC_LISTS[idx])
            ]
            if same_ec1_neighbors and not any(
                ec.split(".")[0] in pred_ec1s
                for idx in same_ec1_neighbors
                for ec in _EC_LISTS[idx]
            ):
                continue

            # Skip if no neighbor supports the actual EC — the reaction may be
            # genuinely unique rather than misannotated
    ...
    print(
        f"  Misannotation candidates (prefilter, confidence >= 0.5): {len(misannotations)}"
    )
    for m in misannotations[:5]:
        print(
            f"    {m['query_id']}: actual={m['actual_ec']}  predicted={m['predicted_ecs']}  conf={m['confidence']:.3f}"
        )
    print()
