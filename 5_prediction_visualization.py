#!/usr/bin/env python
"""
Step 5 — Visualize EC predictions for unannotated reactions and
misannotation candidates.

Generates two figures:
  1. unannotated_predictions.png  — 6-panel overview of predicted ECs
  2. misannotation_analysis.png   — 6-panel analysis of misannotation candidates

Usage:
    conda activate rxnfp
    python 5_prediction_visualization.py
"""
...
def _safe_ec1(ec_str):
    """Extract the first digit of an EC number, or None."""
...
def _safe_ec2(ec_str):
    """Extract EC subclass (e.g. '1.4') from an EC number, or None."""
    if not ec_str or ec_str == "None":
        return None
...
# ─────────────────────────────────────────────────────────────────────────────
# Figure 1: Unannotated Predictions
# ─────────────────────────────────────────────────────────────────────────────
...
def make_unannotated_figure(df_u, output_path):
    ...
    # Filter to reactions with a prefilter prediction
    has_pf = df_u[
    ...
    ax.set_title(
        "Predicted EC Class Distribution\n(prefilter)",
        fontsize=12,
        fontweight="bold",
        pad=10,
    )
    ...
    _style_ax(ax)
    both = df_u[
        df_u["vanilla_predicted_ec"].notna()
        & (df_u["vanilla_predicted_ec"] != "None")
        & df_u["prefilter_predicted_ec"].notna()
        & (df_u["prefilter_predicted_ec"] != "None")
    ].copy()
    agree = both["vanilla_predicted_ec"] == both["prefilter_predicted_ec"]
    ax.scatter(
        both.loc[~agree, "vanilla_confidence"] * 100,
        both.loc[~agree, "prefilter_confidence"] * 100,
        s=8,
        alpha=0.3,
        c="#999999",
        label=f"Disagree (n={int((~agree).sum()):,})",
        rasterized=True,
    )
    ax.scatter(
        both.loc[agree, "vanilla_confidence"] * 100,
    ...
    ax.set_title(
        "Vanilla vs Prefilter Agreement", fontsize=12, fontweight="bold", pad=10
    )
    ...
    box_data = [
        has_pf.loc[has_pf["ec1"] == c, "prefilter_confidence"].values * 100
        for c in ec1_order
    ]
    bp = ax.boxplot(
        box_data,
        labels=[f"EC {c}" for c in ec1_order],
        patch_artist=True,
        widths=0.6,
    ...
    ax.set_title(
        "% Neighbor Agreement by EC Class\n(prefilter)",
        fontsize=12,
        fontweight="bold",
        pad=10,
    )
    ...
    _style_ax(ax)
    max_n = max(
        has_van["vanilla_n_annotated_neighbours"].max(),
        has_pf["prefilter_n_annotated_neighbours"].max(),
    )
    bins = np.arange(0, min(max_n + 2, 32), 1)
    ax.hist(
        has_van["vanilla_n_annotated_neighbours"],
        bins=bins,
        alpha=1.0,
    ...
    ax.hist(
        has_pf["prefilter_n_annotated_neighbours"],
        bins=bins,
        alpha=0.85,
    ...
    ax.set_ylabel("Count", fontsize=10)
    ax.set_title(
        "Annotated Neighbours per Query", fontsize=12, fontweight="bold", pad=10
    )
    ...
    ax.set_ylabel("Count", fontsize=10)
    ax.set_title(
        "Cosine Similarity Distribution", fontsize=12, fontweight="bold", pad=10
    )
    ax.legend(fontsize=9, frameon=False)
...
# ─────────────────────────────────────────────────────────────────────────────
# Figure 2: Misannotation Analysis
# ─────────────────────────────────────────────────────────────────────────────
...
def make_misannotation_figure(df_mis, df_eval, output_path):
    ...
    ax.set_xticklabels(
        [f"EC {c}" for c in all_classes], fontsize=9, rotation=45, ha="right"
    )
    ...
    bars = ax.barh(
        y_pos, top15_counts, color=COL_V, edgecolor="white", linewidth=0.5, height=0.7
    )
    ax.set_yticks(y_pos)
    ax.set_yticklabels(top15_labels, fontsize=7)
    ax.set_xlabel("Count", fontsize=10)
    ax.set_title(
        "Top 15 Reaction Centers\n(misannotation candidates)",
        fontsize=12,
        fontweight="bold",
        pad=10,
    )
    ...
    ax.set_xlabel("Count", fontsize=10)
    ax.set_title(
        "EC Subclass Breakdown\n(top 3 misannotated classes)",
        fontsize=12,
        fontweight="bold",
        pad=10,
    )
    # Legend for top 3 classes
    from matplotlib.patches import Patch

    legend_elements = [
        Patch(
            facecolor=class_colors[i],
            label=f"EC {ec1_top3[i][0]} {EC1_NAMES.get(ec1_top3[i][0], '')}",
        )
    ...
    ax = axes[1, 2]
    _style_ax(ax)
    # Match misannotation candidates to evaluation_results for cosine sim
    mis_ids = set(df_mis["query_id"])
    eval_mis = df_eval[df_eval["query_id"].isin(mis_ids)].copy()
    if len(eval_mis) > 0:
        ax.scatter(
            eval_mis["prefilter_cos_mean"],
            eval_mis["prefilter_majority_confidence"] * 100,
            s=12,
            alpha=0.4,
            color=COL_P,
            rasterized=True,
        ...
        ax.set_ylabel("% Neighbor Agreement", fontsize=10)
        ax.set_title(
            "Cosine Similarity vs % Neighbor Agreement\n(misannotation candidates)",
            fontsize=12,
            fontweight="bold",
            pad=10,
        )
        # Add correlation
        corr = np.corrcoef(
            eval_mis["prefilter_cos_mean"], eval_mis["prefilter_majority_confidence"]
        )[0, 1]
        ax.text(
            0.05,
            0.95,
        ...
        ax.set_title(
            "Cosine Similarity vs % Neighbor Agreement", fontsize=12, fontweight="bold"
        )
...
def main():
    print("Loading data ...")
    df_u = pd.read_csv(os.path.join(BASE_DIR, "ec_predictions_unannotated.csv"))
    df_mis = pd.read_csv(os.path.join(BASE_DIR, "misannotation_candidates.csv"))
    df_eval = pd.read_csv(os.path.join(BASE_DIR, "evaluation_results.csv"))

    print(f"Unannotated predictions: {len(df_u):,}")
    print(f"Misannotation candidates: {len(df_mis):,}")

    out1 = os.path.join(BASE_DIR, "unannotated_predictions.png")
    out2 = os.path.join(BASE_DIR, "misannotation_analysis.png")

    print("\nGenerating unannotated predictions figure ...")
    make_unannotated_figure(df_u, out1)

    print("Generating misannotation analysis figure ...")
    make_misannotation_figure(df_mis, df_eval, out2)
